...
// Find and replace just the COLOR_MAP section using a regex
c = c.replace(
  /const COLOR_MAP = \{[\s\S]*?\};(\s*const isColorOption[\s\S]*?return COLOR_MAP\[key\] \|\| null;\s*\};)/,
  `const COLOR_MAP = {
  ...
  violet: '#7c3aed', cobalt: '#1e40af', crimson: '#dc143c', fuchsia: '#d946ef',
};
...
// Fix onClick for color swatches
c = c.replace(
  /onClick=\{\(\) => setSelectedOptions\(\(o\) => \(\{ \.\.\.o, \[option\.name\]: val \}\)\)\}/g,
  `onClick={() => {
                        setSelectedOptions((o) => ({ ...o, [option.name]: val }));
...
console.log('saltwater:', c.includes('saltwater'));
console.log('colorLower:', c.includes('colorLower'));
